/////////////////////////////////////////////////////////////////////////////////////////
///////////                   GEF MissionCreate Framework 1.0
///////////                            David Maus
/////////////////////////////////////////////////////////////////////////////////////////
// init.sqf - Wird beim Start der Mission von Client und Server ausgeführt


#include "config\missionConfig.sqf"
// Wait until player is initialized
if (!isDedicated) then {waitUntil {!isNull player && isPlayer player};};

/////////////////////////////////////////////////////////////////////////////////////////
// -------------------------------------- Scriptausführung
/////////////////////////////////////////////////////////////////////////////////////////
root = parsingNamespace getVariable "MISSION_ROOT";
enableSaving [ false, false ]; // Disable Save und Autosave







/////////////////////////////////////////////////////////////////////////////////////////
/////////// R3F Logistics initalisieren
if (r3fLogistic == 1) then
{
[] execVM "scripts\other\R3F_LOG\init.sqf";
};
/////////////////////////////////////////////////////////////////////////////////////////


/////////////////////////////////////////////////////////////////////////////////////////
/////////// R3F Logistics initalisieren
if (HelmGogglesEFX == 1) then
{
[] execVM "scripts\other\HelmAndGoggles_Extended_DM\init.sqf";
};
/////////////////////////////////////////////////////////////////////////////////////////


/////////////////////////////////////////////////////////////////////////////////////////
/////////// R3F Logistics initalisieren
if (SafeZoneMapObjs == 1) then
{
['SafeZone_'] execVM "scripts\other\safeMapObjects.sqf";
};
/////////////////////////////////////////////////////////////////////////////////////////


/////////////////////////////////////////////////////////////////////////////////////////
/////////// R3F Logistics initalisieren
if (DestroyBuildings == 1) then
{
['DestroyBuilding_'] execVM "scripts\other\DestroyBuildings.sqf";
};
/////////////////////////////////////////////////////////////////////////////////////////


/////////////////////////////////////////////////////////////////////////////////////////
/////////// R3F Logistics initalisieren
if (GasZone == 1) then
{
[GasZoneMarkerName, GasZoneDynamicMarkerName, GasFog_Mindistance, GasFog_CloudCount, GasFog_CloudMinSize, GasFog_CloudMaxSize, GasFog_CloudHeight, GasFog_CloudRespawn, GasFog_Color, GasFog_Transparency, GasFog_CloudRemoveSpeed, GasFog_CloudMinLifetime, GasFog_CloudMaxLifetime, GasFog_AffectedByWind, GasFog_WindStrenght, GasFog_RandDirTime, GasFog_RandDirStrenght, GasFog_MoveDirectionSpeed] execVM "scripts\other\GasMask_DM\init.sqf";
};
/////////////////////////////////////////////////////////////////////////////////////////

/////////////////////////////////////////////////////////////////////////////////////////
/////////// R3F Logistics initalisieren
if (HaloStart == 1) then
{
[HaloStartName, HaloStartHeight, HaloTargetName, HaloCargoOpenTime] execVM "scripts\other\HALO_DM\init.sqf";
};
/////////////////////////////////////////////////////////////////////////////////////////


/////////////////////////////////////////////////////////////////////////////////////////
/////////// Server Info Menu aktivieren
if (serverInfoMenu == 1) then
{
[] ExecVM "scripts\other\scarCODE\ServerInfoMenu\sqf\initLocal.sqf"; // scarCODE ServerInfoMenu
};
/////////////////////////////////////////////////////////////////////////////////////////




/////////////////////////////////////////////////////////////////////////////////////////
/////////// Postprocess Editor initalisieren
if (mission == 1) then
{
[] execVM "mission\mission.sqf";
};
/////////////////////////////////////////////////////////////////////////////////////////



/////////////////////////////////////////////////////////////////////////////////////////
/////////// Postprocess Editor initalisieren
if (briefing == 1) then
{
[] execVM "mission\briefing.sqf";
};
/////////////////////////////////////////////////////////////////////////////////////////




/////////////////////////////////////////////////////////////////////////////////////////
/////////////////// Intro
if (intro == 1) then
{

[] execVM "mission\intro.sqf";


};
/////////////////////////////////////////////////////////////////////////////////////////




/////////////////////////////////////////////////////////////////////////////////////////
/////////////////// PostEffects
if (postEffects == 1) then
{
[] execVM "config\missionColor.sqf";
};
/////////////////////////////////////////////////////////////////////////////////////////




/////////////////////////////////////////////////////////////////////////////////////////
/////////// Postprocess Editor initalisieren
if (postProcessEditor == 1) then
{
[] execVM "scripts\other\GFPPE\GFPPE_init.sqf";
};
/////////////////////////////////////////////////////////////////////////////////////////




/////////////////////////////////////////////////////////////////////////////////////////
/////////// Alive Auto-Tasks filtern
if (aliveConfig == 1) then
{
[] execVM "config\aliveConfig.sqf";
};
/////////////////////////////////////////////////////////////////////////////////////////




/////////////////////////////////////////////////////////////////////////////////////////
/////////// Advanced Slingloading (Mod) Einstellungen
if (aslAll == 1) then
{
ASL_SLING_RULES_OVERRIDE = [
      ["All","CAN_SLING","All"]
];

ASL_HEAVY_LIFTING_ENABLED = true;
};
/////////////////////////////////////////////////////////////////////////////////////////




/////////////////////////////////////////////////////////////////////////////////////////
/////////// Day & Night Time Multiplicator
/////////// [DayMultiply,NightMultiply] execVM "scripts\other\DayNightTimer.sqf";
if (timeMultiply == 1) then
{
[dayMultiply,nightMultiply] execVM "scripts\other\DayNightTimer.sqf";
};
/////////////////////////////////////////////////////////////////////////////////////////




/////////////////////////////////////////////////////////////////////////////////////////
/////////// Virtual Arsenal mit Insignia, Faces und Voice auf alle Kisten mit angegebenen Namen
/////////// ["Name der Kiste mit Zahl",Anzahl der Kisten)] execVM "scripts\other\vArsenalMaker.sqf";
if (vArsenalMaker == 1) then
{
["vArsenal_"] execVM "scripts\other\vArsenalMaker.sqf";
};
/////////////////////////////////////////////////////////////////////////////////////////

/////////////////////////////////////////////////////////////////////////////////////////
/////////// Virtual Arsenal mit Insignia, Faces und Voice auf alle Kisten mit angegebenen Namen
/////////// ["Name der Kiste mit Zahl",Anzahl der Kisten)] execVM "scripts\other\vArsenalMaker.sqf";
if (LightSourceMaker == 1) then
{
["LightSource_"] execVM "scripts\other\LightSourceMaker.sqf";
};
/////////////////////////////////////////////////////////////////////////////////////////

/////////////////////////////////////////////////////////////////////////////////////////
/////////// Virtual Arsenal mit Insignia, Faces und Voice auf alle Kisten mit angegebenen Namen
/////////// ["Name der Kiste mit Zahl",Anzahl der Kisten)] execVM "scripts\other\vArsenalMaker.sqf";
if (DeadBodyMaker == 1) then
{
["DeadBody_"] execVM "scripts\other\DeadBodyMaker.sqf";
};
/////////////////////////////////////////////////////////////////////////////////////////




/////////////////////////////////////////////////////////////////////////////////////////
/////////////////// Entfernt alle nvgs und Ir Laser von Opfor im 30 sekunden Interval
if (vcom == 1) then
{
If (isServer || !(hasinterface) ) then {[] execVM "Vcom\VcomInit.sqf";}
};
/////////////////////////////////////////////////////////////////////////////////////////




/////////////////////////////////////////////////////////////////////////////////////////
/////////// TASK FORCE RADIO Config
/////////// Hier Task Force Radio einstellen bei Bedarf
if (disableVCOMAIforAIR == 1) then
{
[] execVM "scripts\other\disableVCOMAIforAIR.sqf";
};
/////////////////////////////////////////////////////////////////////////////////////////




/////////////////////////////////////////////////////////////////////////////////////////
/////////////////// Rotes Interieur Licht für Laderäume etc. in Helis z.b.
/////////////////// [LichtMultiplier] execVM "scripts\other\IntLight.sqf";
if (interiorLight == 1) then
{
[interiorLightMultiply] execVM "scripts\other\IntLight.sqf";
};
/////////////////////////////////////////////////////////////////////////////////////////




/////////////////////////////////////////////////////////////////////////////////////////
/////////////////// Foglimiter (Alive Missionen mit dem Weather Modul können zu starken Nebel erzeugen)
/////////////////// [Zeitinterval der Checks, Maximaler Nebel] execVM "scripts\other\FogLimiter.sqf";
if (fogLimiter == 1) then
{
[fogLimiterInterval, fogLimiterMax] execVM "scripts\other\FogLimiter.sqf";
};
/////////////////////////////////////////////////////////////////////////////////////////




/////////////////////////////////////////////////////////////////////////////////////////
/////////////////// ThirdPerson nur in der Zone (z.b. Basis) erlaubt
/////////////////// ["Markername"] execVM "scripts\other\CameraView.sqf";
if (thirdPersonInBase == 1) then
{
[thirdPersonInBaseMarker] execVM "scripts\other\CameraView.sqf";
};
/////////////////////////////////////////////////////////////////////////////////////////




/////////////////////////////////////////////////////////////////////////////////////////
/////////////////// Entfernt alle nvgs und Ir Laser von Opfor im 30 sekunden Interval
if (nvgOffLightOn == 1) then
{
[] execVM "scripts\other\nvgOffLightsOn.sqf";
};
/////////////////////////////////////////////////////////////////////////////////////////


/////////////////////////////////////////////////////////////////////////////////////////
/////////////////// Entfernt alle nvgs und Ir Laser von Opfor im 30 sekunden Interval
if (removeGrenades == 1) then
{
[] execVM "scripts\other\removeGrenades.sqf";
};
/////////////////////////////////////////////////////////////////////////////////////////


/////////////////////////////////////////////////////////////////////////////////////////
/////////////////// Entfernt alle nvgs und Ir Laser von Opfor im 30 sekunden Interval
if (tpw == 1) then
{
[[""]] execvm "scripts\other\TPW\tpw_core.sqf";
[60,1,0] execvm "scripts\other\TPW\tpw_radio.sqf";
[1,1,1,1,1,0,0,0] execVM "scripts\other\TPW\tpw_soap.sqf";
[4,18,5,25,5,50,5,0.1,[250,220,100],0.8,0.05,1,0.075] execvm "scripts\other\TPW\tpw_fireflies.sqf";
};
/////////////////////////////////////////////////////////////////////////////////////////



/////////////////////////////////////////////////////////////////////////////////////////
/////////////////// Entfernt alle nvgs und Ir Laser von Opfor im 30 sekunden Interval
if (dynamicFurniture == 1) then
{
[] execVM "scripts\other\PF\init.sqf";
};
/////////////////////////////////////////////////////////////////////////////////////////


/////////////////////////////////////////////////////////////////////////////////////////
/////////////////// Entfernt alle nvgs und Ir Laser von Opfor im 30 sekunden Interval
if (Teleport == 1) then
{
[] execVM "scripts\other\teleport.sqf";
};
/////////////////////////////////////////////////////////////////////////////////////////



/////////////////////////////////////////////////////////////////////////////////////////
/////////////////// Entfernt alle nvgs und Ir Laser von Opfor im 30 sekunden Interval
if (noProne > 0) then
{
[noProne] execVM "scripts\other\noProne.sqf";
};
/////////////////////////////////////////////////////////////////////////////////////////


/////////////////////////////////////////////////////////////////////////////////////////
/////////////////// Entfernt alle nvgs und Ir Laser von Opfor im 30 sekunden Interval
if (lockDoors == 1) then
{
['LockDoors_'] execVM "scripts\other\lockDoors.sqf";
};
/////////////////////////////////////////////////////////////////////////////////////////


/////////////////////////////////////////////////////////////////////////////////////////
/////////////////// Entfernt alle nvgs und Ir Laser von Opfor im 30 sekunden Interval
if (explosions == 1) then
{
['ExplosionsRandom'] execVM "scripts\other\explosionsRandom.sqf";
};
/////////////////////////////////////////////////////////////////////////////////////////


/////////////////////////////////////////////////////////////////////////////////////////
/////////////////// Entfernt alle nvgs und Ir Laser von Opfor im 30 sekunden Interval
if (VolumetricFog == 1) then
{
    if (!isDedicated && hasInterface) then
    {
        null=[player,VolFog_MaxDistance,VolFog_Mindistance,VolFog_CloudCount,VolFog_CloudMinSize,VolFog_CloudMaxSize,VolFog_CloudHeight,VolFog_CloudRespawn,VolFog_Transparency,VolFog_Color select 0,VolFog_Color select 1,VolFog_Color select 2,VolFog_CloudRemoveSpeed,VolFog_CloudMinLifetime,VolFog_CloudMaxLifetime,VolFog_AffectedByWind,VolFog_WindStrenght,VolFog_RandDirTime,VolFog_RandDirStrenght,VolFog_MoveDirectionSpeed select 0,VolFog_MoveDirectionSpeed select 1,VolFog_MoveDirectionSpeed select 2,VolFog_StartTime,VolFog_Endtime] execFSM "scripts\other\GasMask_DM\Fog.fsm";

        player addEventHandler ["Respawn", {

            null=[player,VolFog_MaxDistance,VolFog_Mindistance,VolFog_CloudCount,VolFog_CloudMinSize,VolFog_CloudMaxSize,VolFog_CloudHeight,VolFog_CloudRespawn,VolFog_Transparency,VolFog_Color select 0,VolFog_Color select 1,VolFog_Color select 2,VolFog_CloudRemoveSpeed,VolFog_CloudMinLifetime,VolFog_CloudMaxLifetime,VolFog_AffectedByWind,VolFog_WindStrenght,VolFog_RandDirTime,VolFog_RandDirStrenght,VolFog_MoveDirectionSpeed select 0,VolFog_MoveDirectionSpeed select 1,VolFog_MoveDirectionSpeed select 2,VolFog_StartTime,VolFog_Endtime] execFSM "scripts\other\GasMask_DM\Fog.fsm";


        }];

};
};
/////////////////////////////////////////////////////////////////////////////////////////



/////////////////////////////////////////////////////////////////////////////////////////
/////////////////// Entfernt alle nvgs und Ir Laser von Opfor im 30 sekunden Interval
if (CaptureUnit == 1) then
{
[] execVM "scripts\other\infUnitCapture\addActionCapture.sqf";
};
/////////////////////////////////////////////////////////////////////////////////////////


/////////////////////////////////////////////////////////////////////////////////////////
/////////////////// Entfernt alle nvgs und Ir Laser von Opfor im 30 sekunden Interval
if (nvgBattery == 1) then
{
[nvgBatteryTime] execVM "scripts\other\nvgBattery.sqf";
};
/////////////////////////////////////////////////////////////////////////////////////////

/////////////////////////////////////////////////////////////////////////////////////////
/////////////////// Entfernt alle nvgs und Ir Laser von Opfor im 30 sekunden Interval
if (idSystem == 1) then
{
 if (isServer) then {
     [
         [west,westFaction],
         [east,eastFaction],
         [independent,indFaction],
         [civilian,civFaction]
     ] call GEF_fnc_idSystem;
 };
};

// Hier unten anderen Code und andere Scripte ausführen

